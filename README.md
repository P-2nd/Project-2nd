...
* **프로젝트 소개**
    * 100만 건 규모의 헬스장 회원 데이터를 기반으로, 회원의 인구통계·이용 패턴·행동 이력 데이터를 분석하여 이탈(Churn) 여부를 예측하는 프로젝트입니다.
    * EDA를 통해 이탈에 실질적으로 영향을 주는 변수(연체 건수, 방문 빈도, 참여도 지표 등)와 영향이 없는 변수(성별, 요금제, 선호 시간대 등)를 구분합니다.
    * 데이터 전처리(결측치 처리, 인코딩, 스케일링) 및 Feature Selection을 거쳐 여러 머신러닝/딥러닝 모델(Logistic Regression, XGBoost, Deep Learning 등)의 성능을 비교합니다.
...
이번 프로젝트는 여러모로 아쉬움이 많이 남았습니다.

다양한 머신러닝 모델을 학습하고 비교하며 최적의 모델을 선정하는 과정을 직접 경험해볼 수 있었던 점은 큰 수확이었습니다. 또한 단순히 모델을 만드는 것에 그치지 않고, 데이터 전처리부터 성능 평가까지 전체 머신러닝 프로젝트의 흐름을 이해할 수 있었습니다.

다만 프로젝트를 진행하면서 데이터 전처리와 모델 튜닝, 성능 개선 측면에서 부족한 부분이 많다는 것을 느꼈습니다. 이번 프로젝트 산출물을 기반으로 부족했던 부분을 다시 학습하고, 다양한 기법을 적용하며 성능을 개선해보는 시간을 가져야겠다는 생각이 들었습니다. 이를 통해 다음 프로젝트에서는 더욱 완성도 높은 결과물을 만들 수 있도록 노력하고자 합니다.

### 오호민 

#### **담당 업무**

* 프로젝트 전체 데이터 파이프라인 설계
* 데이터 전처리 및 EDA(탐색적 데이터 분석)
* Feature Selection 및 전처리 데이터셋 구축

#### **회고**

프로젝트의 데이터 파이프라인을 설계하며 데이터 로드부터 전처리, 모델 학습, 평가까지의 전체 흐름을 일관성 있게 구성했습니다. 또한 결측치 처리, 인코딩, 스케일링, Feature Selection을 수행하고 EDA를 통해 회원 이탈에 영향을 미치는 주요 변수를 분석하며 모델 학습에 적합한 데이터를 구축했습니다.

이번 프로젝트를 통해 데이터 분석은 모델 성능 향상뿐 아니라 데이터의 특성을 이해하고 적절한 전처리 전략을 수립하는 과정이 중요하다는 점을 배웠습니다. 또한 여러 모델이 동일한 전처리 데이터를 활용할 수 있도록 파이프라인을 구성하면서 재현성과 유지보수성을 고려한 데이터 처리의 중요성을 경험할 수 있었습니다.
...
* 프로젝트 개발 환경 설정 및 Git 협업 흐름 관리 
* README, GUIDE, RESULT 문서 작성 및 실행 절차 정리
* 모델 평가 결과 추출·정리
* 모델 구현 및 성능 비교

#### **회고**

MLP 모델을 개발하며 트리 기반 모델들과는 다른 관점에서 데이터를 다뤄야 한다는 점을 배웠습니다. 레이어 구조와 하이퍼파라미터를 조정해 가며 학습을 진행했고, 다른 팀원들이 구현한 KNN, Logistic Regression, XGBoost, LightGBM, Random Forest 결과와 비교하며 모델별 성능 차이가 나타나는 이유를 분석했습니다.

이 과정에서 Accuracy만으로는 모델을 평가할 수 없으며, 클래스 불균형이 있는 데이터에서는 Precision, Recall, F1 Score, ROC-AUC를 함께 살펴봐야 한다는 점을 체감했습니다. 또한 프로젝트 결과를 정리해 발표 자료를 제작하고 발표하면서, 분석 과정과 수치를 팀 외부 사람도 이해할 수 있도록 논리적으로 전달하는 능력의 중요성을 다시 한번 느꼈습니다.
...
#### **회고**
  ...
  프로젝트 환경 설정과 Git 협업 흐름을 관리하며, 팀원 모두가 동일한 환경에서 안정적으로 작업할 수 있
  도록 구성했습니다. 또한 README, GUIDE, RESULT 문서를 작성해 프로젝트 구조, 실행 방법, 결과 확인 절
  차를 명확히 정리했습니다.

  KNN과 Logistic Regression 모델을 구현·평가하면서, 모델 성능은 정확도만으로 판단할 수 없다는 점을 배
  웠습니다. 특히 대용량 데이터에서는 F1-score와 Recall 같은 분류 성능뿐 아니라, 실제 서비스 환경에서
  필요한 학습·추론 시간도 함께 고려해야 함을 체감했습니다.
